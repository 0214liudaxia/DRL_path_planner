...
class Env():
    def __init__(self, is_training, train_env_id, test_env_id=2, visual_obs=False, num_scan_ranges=10):
        ...
        if self.test_env_id == 4:
            self.test_goals = [(2.,2.), (-2.5,1.), (0.,-2.), (-1., -1.2)]
        ...
        elif self.test_env_id == 5:
            #self.test_goals = [(-0.9,4.25),(-3.6, 5.3),(-3.0,0),(0.0, -3.3),(4.8,-3.9),(6.0,-2.1) , (0.,0.)]#(-3.6, 5.3)
            self.test_goals = [(-0.9,4.25),(-3.6, 4.3),(-3.0,0),(0.0, -3.3),(4.8,-3.9),(6.0,-2.1) , (0.,0.)]#(-0.0,2.25),(-3.6, 2.3),
    ...
    def getState(self, scan, image):
        ...
        if self.visual_obs:
            ...
            #rgba = bytes(image)
            #image = PIL.Image.frombytes('RGB', (228, 304), rgba)
            #image = PIL.Image.fromarray(image,mode='RGBA')
        
            #image = PIL.Image.fromarray(np.uint8(image).reshape(228,304,3),'RGB')
            pix = np.array(image)
            #image = PIL.Image.new('RGB', (w, h,3))
            # Copy the pixel data to the Image
            #image.putdata(pix)
            
            image = PIL.Image.fromarray(pix)#.reshape(w,h,3),mode='RGBA'
